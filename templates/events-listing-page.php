			...
			<div class="container reveal" style="max-width: var(--container); margin: 0 auto; padding: 0 24px;">
				...
				if ($section_type === 'media') {
					...
					if ($section['mediaType'] === 'photo' && !empty($section['photo_src'])) {
						...
						$media_col_html = '<div class="wp-block-column map panel reveal" style="background:#fff;padding:18px;border-radius:12px;box-shadow:var(--shadow)"><iframe width="100%" height="100%" style="min-height:320px;border:0;border-radius:12px;" loading="lazy" src="' . esc_url($section['map_embed_src']) . '" title="Venue map"></iframe></div>';
					}

					$content_col_html = '<aside class="wp-block-column panel reveal" style="background:#fff;padding:18px;border-radius:12px;box-shadow:var(--shadow)">';
...
// PR-2: Query 'event' CPT instead of pages
...
if ($all_events_query->have_posts()) {
    while ($all_events_query->have_posts()) {
        ...
        $event_place = get_post_meta(get_the_ID(), 'event_location', true);
    ...
    <header class="nav" role="banner">
      <div class="container nav-inner">
        ...
        <nav class="nav-links" role="navigation" aria-label="Primary">
            <?php
                // This is the global events page, so we use a static navigation bar.
                echo '<div class="nav-links-main" style="flex-grow: 1;">';
                echo '<a href="' . esc_url(get_permalink(get_page_by_path('events'))) . '" style="background: #0000000f; font-weight: 700;">Upcoming Events</a>';
                echo '<a href="' . esc_url(get_permalink(get_page_by_path('past-events'))) . '">Past Events</a>';
                echo '</div>';
            ?>
            <div class="nav-links-secondary">
                <a href="<?php echo esc_url( get_permalink( get_page_by_path( 'code-of-conduct' ) ) ); ?>">Code of Conduct</a>
            ...
            <div class="page-layout">
                <div class="main-content">
                    ...
                    <div id="events-container">
                        <?php
                        if ( $all_events_query->have_posts() ) :
                            while ( $all_events_query->have_posts() ) : $all_events_query->the_post(); // Using the new CPT query
                                $event_date = get_post_meta( get_the_ID(), 'event_date', true );
                                $event_end_date = get_post_meta( get_the_ID(), 'event_end_date', true );
                                $event_place = get_post_meta( get_the_ID(), 'event_location', true );
                                $event_description = get_the_excerpt();
                                ...
                                if (!empty($event_date)) {
                                    ...
                                    if (!empty($event_end_date) && $event_end_date !== $event_date) { // Corrected logic for date formatting
                        ...
                        ?>
                            <div class="event-card" 
                                ...
                                data-time="<?php echo esc_attr(get_post_meta( get_the_ID(), 'event_time', true )); ?>"
                                data-end-date="<?php echo esc_attr($event_end_date); ?>"
                                data-description="<?php echo esc_attr($event_description); ?>"
                                data-lead-text="<?php echo esc_attr(get_post_meta( get_the_ID(), 'event_lead_text', true )); ?>"
                                data-registration-link="<?php echo esc_attr(get_post_meta( get_the_ID(), 'event_registration_link', true )); ?>"
                                data-cfs-link="<?php echo esc_attr(get_post_meta( get_the_ID(), 'event_cfs_link', true )); ?>"
                    ...
                    <?php render_custom_sections('events_after_cards', $custom_sections_data); ?>
                </div>
                <aside class="sidebar">
                    <h2>Latest News</h2>
...
class EventsPageManager {
    constructor(ajaxUrl, nonce) {
        ...
        this.nonce = nonce;
        // Safely initialize elements that might not exist on every page load
    ...
    handleFormResponse(data) {
        if (data.success) {
            ...
            this.closeModal();
            this.eventForm.reset();
            window.location.reload(); // Reload to show the new card in its correct sorted position.
        } else {
            alert('Error: ' + data.data);
        }
