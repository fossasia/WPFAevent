				...
				if ($layout === 'full_width') {
					...
					<div class="panel">
						...
						<?php if (!empty($section['contentBody'])) : ?>
							<div><?php echo wp_kses_post($section['contentBody']); ?></div>
						<?php endif; ?>
					...
					if (!empty($section['contentBody'])) {
						$content_col_html .= '<div>' . wp_kses_post($section['contentBody']) . '</div>';
					}
...
$hero_image_url = plugins_url( '../images/hero-image.jpg', __FILE__ ); // Default
...
<style>
  ...
  *{box-sizing:border-box}html,body{height:100%;margin:0;background:var(--bg);color:var(--text);font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif}
  ...
  /* CTA + footer */
  .cta{padding:54px 0;background:linear-gradient(180deg,#fff7f6,#fff)}
    ...
    <nav class="nav-links" role="navigation" aria-label="Primary">
      ...
      <?php
          if ( current_user_can( 'manage_options' ) ) {
              // User is an admin, link to dashboard with a return URL.
              $dashboard_url = get_permalink( get_page_by_path( "admin-dashboard" ) );
              $return_url = add_query_arg( 'return_to', urlencode( get_permalink() ), $dashboard_url );
              echo '<a href="' . esc_url( $return_url ) . '">Admin Dashboard</a>';
          } else {
...
<section class="wp-block-group alignfull hero" aria-labelledby="hero-title">
    ...
    <div class="wp-block-group container hero-inner">
        ...
        <p class="lead">
            <?php echo esc_html( $event_lead_text ?: 'Three days of talks, workshops and community engagement. Join maintainers, contributors and teams from across open-source projects and industry.' ); ?>
        </p>
        <!-- /wp:paragraph -->
...
<!-- wp:group {"tagName":"section","className":"wp-block-group","metadata":{"name":"About Summit"}} -->
<section id="about" aria-labelledby="about-title" class="wp-block-group">
    <!-- wp:group {"className":"container reveal","layout":{"type":"constrained"}} -->
    <div class="wp-block-group container reveal">
...
<!-- wp:group {"tagName":"section","layout":{"type":"constrained"},"className":"wp-block-group","metadata":{"name":"Speakers"}} -->
<section id="speakers" class="wp-block-group">
    ...
    <div class="wp-block-group has-global-padding-is-contained container" style="padding-top:40px;padding-right:20px;padding-bottom:40px;padding-left:20px;max-width:1200px;margin-left:auto;margin-right:auto;">
        <!-- wp:group {"className":"section-head","layout":{"type":"flex","flexWrap":"nowrap","justifyContent":"space-between"},"style":{"spacing":{"margin":{"bottom":"30px"}}}} -->
        <div class="wp-block-group section-head" style="margin-bottom:30px;">
            <!-- wp:heading {"level":2,"className":"h2","style":{"typography":{"fontSize":"28px","margin":{"bottom":"10px"}}}} -->
            <h2 class="h2" style="font-size:28px;margin-bottom:10px;">Featured Speakers</h2>
            <!-- /wp:heading -->
            <!-- wp:paragraph {"className":"meta","align":"center","style":{"color":{"text":"#5b636a"}}} -->
            <p class="has-text-align-center meta" style="color:#5b636a;">Curated invited & selected speakers</p>
            <!-- /wp:paragraph -->
        </div>
        <!-- /wp:group -->
...
<!-- wp:group {"tagName":"section","layout":{"type":"constrained"},"className":"wp-block-group","metadata":{"name":"Schedule Overview"}} -->
<section id="schedule-overview" class="wp-block-group">
    <!-- wp:group {"className":"container","layout":{"type":"constrained"}} -->
    <div class="wp-block-group container">
        <!-- wp:group {"className":"section-head","layout":{"type":"flex","flexWrap":"nowrap","justifyContent":"space-between"}} -->
        <div class="wp-block-group section-head">
            <!-- wp:heading {"level":2,"className":"h2"} -->
            <h2 class="h2">Schedule Overview</h2>
            <!-- /wp:heading -->
        </div>
        <!-- /wp:group -->
        ...
        <!-- wp:html -->
        <?php
            $schedule_file = $upload_dir['basedir'] . '/fossasia-data/schedule.json';
...
<!-- wp:group {"tagName":"section","layout":{"type":"constrained"},"className":"wp-block-group","metadata":{"name":"Sponsors"}} -->
<section id="sponsors" class="wp-block-group">
    <!-- wp:group {"className":"container","layout":{"type":"constrained"}} -->
    <div class="wp-block-group container">
        <!-- wp:group {"className":"section-head","layout":{"type":"flex","flexWrap":"nowrap","justifyContent":"space-between"}} -->
        <div class="wp-block-group section-head">
...
<!-- wp:group {"tagName":"section","layout":{"type":"constrained"},"className":"wp-block-group","metadata":{"name":"Venue & Travel"}} -->
<section id="venue" class="wp-block-group">
    <!-- wp:group {"className":"container","layout":{"type":"constrained"}} -->
    <div class="wp-block-group container">
        <!-- wp:group {"className":"section-head","layout":{"type":"flex","flexWrap":"nowrap","justifyContent":"space-between"}} -->
        <div class="wp-block-group section-head">
            <!-- wp:heading {"level":2,"className":"h2"} -->
            <h2 class="h2">Venue & Travel</h2>
            <!-- /wp:heading -->
            <!-- wp:paragraph {"className":"meta"} -->
            <p class="meta">True Digital Park West, Bangkok • In-person & hybrid</p>
            <!-- /wp:paragraph -->
        </div>
        <!-- /wp:group -->

        <!-- wp:columns {"className":"venue-grid"} -->
        <div class="wp-block-columns venue-grid">
            <!-- wp:column {"className":"map panel reveal"} -->
            <div class="wp-block-column map panel reveal">
                <!-- wp:html -->
                <iframe width="100%" height="100%" style="min-height:320px;border:0" loading="lazy"
                  src="https://www.openstreetmap.org/export/embed.html?bbox=103.8198%2C1.3521%2C103.8298%2C1.3621&amp;layer=mapnik" title="Venue map"></iframe>
                <!-- /wp:html -->
            </div>
            <!-- /wp:column -->

            <!-- wp:column {"tagName":"aside","className":"panel reveal"} -->
            <aside class="wp-block-column panel reveal" aria-labelledby="venue-info">
                <!-- wp:heading {"level":3,"anchor":"venue-info","style":{"spacing":{"margin":{"top":"0"}}}} -->
                <h3 id="venue-info" style="margin-top:0;">Venue details</h3>
                <!-- /wp:heading -->
                <!-- wp:paragraph {"className":"meta"} -->
                <p class="meta">True Digital Park West, Bangkok — easy access from central Bangkok. International attendees arrive via Suvarnabhumi (BKK) or Don Mueang (DMK).</p>
                <!-- /wp:paragraph -->
                <!-- wp:list -->
                <ul>
                    <li>Local hotels within short travel time</li>
                    <li>Accessible venue and facilities</li>
                    <li>Hybrid streaming for remote attendees</li>
                </ul>
                <!-- /wp:list -->
                <!-- wp:buttons {"style":{"spacing":{"margin":{"top":"14px"}}},"className":"hero-ctas"} -->
                <div class="wp-block-buttons hero-ctas" style="margin-top:14px;">
                    <!-- wp:button {"className":"btn btn-primary","url":"<?php echo esc_url($registration_link); ?>","target":"_blank","rel":"noopener"} -->
                    <div class="wp-block-button btn btn-primary"><a class="wp-block-button__link wp-element-button" href="<?php echo esc_url($registration_link); ?>" target="_blank" rel="noopener">Buy Tickets</a></div>
                    <!-- /wp:button -->
                    <!-- wp:button {"className":"btn","url":"https://eventyay.com/e/4c0e0c27/exhibition","target":"_blank","rel":"noopener","style":{"border":{"width":"2px","style":"solid","color":"var(--brand)"},"color":{"text":"var(--brand)"}}} -->
                    <div class="wp-block-button btn" style="border:2px solid var(--brand);color:var(--brand);"><a class="wp-block-button__link wp-element-button" href="https://eventyay.com/e/4c0e0c27/exhibition" target="_blank" rel="noopener">Exhibition</a></div>
                    <!-- /wp:button -->
                </div>
                <!-- /wp:buttons -->
            </aside>
...
<footer class="wp-block-group footer">
    ...
    <?php $global_settings = file_exists($global_settings_file) ? json_decode(file_get_contents($global_settings_file), true) : []; ?>
    <small class="has-text-align-center container center">
        <?php echo esc_html( $global_settings['footer_text'] ?? '© FOSSASIA • FOSSASIA Summit — Mar 13–15, 2025 • True Digital Park West, Bangkok' ); ?>
    </small>
    <!-- /wp:paragraph -->
</footer>
<!-- /wp:group -->
