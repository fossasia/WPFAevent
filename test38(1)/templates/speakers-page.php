...
<div id="page" class="site">
        ...
        <nav class="nav-links" role="navigation" aria-label="Primary">
          ...
          <?php
            if ( current_user_can( 'manage_options' ) ) {
                // User is an admin, link to dashboard.
                $dashboard_url = get_permalink( get_page_by_path( 'admin-dashboard' ) );
                $return_url = $dashboard_url; // Default
                if ($event_id) {
                    // If we have an event context, link to its specific dashboard
                    $return_url = add_query_arg( 'event_id', $event_id, $dashboard_url );
                }
    ...
    <main>
        ...
        <div class="results-info">
            <span id="resultsCount">0</span> speakers found
        </div>
