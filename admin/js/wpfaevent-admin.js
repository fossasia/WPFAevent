	...
	$(function() {
		...
		if ($importForm.length || $updateForm.length) {
			...
			$form.on('submit', function(e) {
				...
				function saveSummaryAndRedirect(type, message, targetPage, summaryNonce) {
					$.ajax({
						url: ajaxurl,
						type: 'POST',
						data: {
						...
						complete: function() {
							window.location.href = 'edit.php?post_type=wpfa_event&page=' + encodeURIComponent(targetPage);
						}
					});
				}
			});
