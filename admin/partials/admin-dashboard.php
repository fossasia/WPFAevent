<?php
/**
 * Admin dashboard template for managing FOSSASIA event content.
 *
 * @package    Wpfaevent
...
// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only event selector from the Events page.
...
if ( $event_id ) {
	...
	// This remains the "global" file for the Events Listing page.
...
// phpcs:disable WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents,WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- Local JSON files under uploads.
// Ensure files exist.
if ( $event_id ) {
...
// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Read-only navigation aids from GET/referer.
if ( isset( $_GET['return_to'] ) ) {
	// Priority 1: Use the 'return_to' query parameter if it exists.
	$return_to     = sanitize_text_field( wp_unslash( $_GET['return_to'] ) );
	$view_site_url = esc_url( urldecode( $return_to ) ); // This is now the event-specific page.
} else {
...
<div class="container">
	<div class="dashboard-header">
		...
		<div class="header-actions">
			...
			<?php endif; ?>
			<a href="<?php echo esc_url( wp_logout_url( home_url( '/fossasia-summit/' ) ) ); ?>" class="btn btn-reject">Logout</a>
		</div>
	</div>
	...
	<section class="dashboard-section" id="main-dashboard-content">
		...
		<div id="panel-settings" class="dashboard-panel">
			<form id="siteSettingsForm">
				...
				<?php
				$default_event_logo = ! empty( $global_settings_data['site_logo_url'] ) ? $global_settings_data['site_logo_url'] : plugins_url( '../assets/images/logo.png', __DIR__ . '/../includes/class-wpfaevent-landing.php' );
				$current_event_logo = isset( $site_settings_data['event_logo_url'] ) && $site_settings_data['event_logo_url'] ? $site_settings_data['event_logo_url'] : $default_event_logo;
				?>
				<img id="currentEventLogo" src="<?php echo esc_url( $current_event_logo ); ?>" alt="Current Event Logo" style="max-height: 36px; height: auto; background: #eee; padding: 5px; border-radius: 4px; margin-bottom: 10px;">
				<br>
...
document.addEventListener('DOMContentLoaded', function() {
	const adminNonce = <?php echo wp_json_encode( wp_create_nonce( 'fossasia_admin_nonce' ) ); ?>;
	const ajaxUrl = <?php echo wp_json_encode( admin_url( 'admin-ajax.php' ) ); ?>;
	const eventId = <?php echo wp_json_encode( (int) $event_id ); ?>;
	
	// --- Data Store ---
	const store = {
		speakers: <?php echo wp_json_encode( $speakers_data ); ?>,
		sponsors: <?php echo wp_json_encode( $sponsors_data ); ?>,
		settings: <?php echo wp_json_encode( $site_settings_data ); ?>,
		sections: <?php echo wp_json_encode( $custom_sections_data ); ?>,
		navigation: <?php echo wp_json_encode( $navigation_data ); ?>,
		schedule: <?php echo wp_json_encode( $schedule_data ); ?>,
		theme: <?php echo wp_json_encode( $theme_settings_data ); ?>
	};
	...
	(() => {
		...
		const render = () => {
			...
			eventLogoPreview.src = store.settings.event_logo_url || <?php echo wp_json_encode( ! empty( $global_settings_data['site_logo_url'] ) ? $global_settings_data['site_logo_url'] : plugins_url( '../assets/images/logo.png', __DIR__ . '/../includes/class-wpfaevent-landing.php' ) ); ?>;
		};
