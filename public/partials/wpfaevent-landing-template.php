<?php
/**
 * Legacy landing template bridge.
 *
 * @package Wpfaevent
 */
...
$speakers_file = $data_dir . '/speakers-' . $event_post_id . '.json';
$sponsors_file = $data_dir . '/sponsors-' . $event_post_id . '.json';
$settings_file = $data_dir . '/site-settings-' . $event_post_id . '.json';
$schedule_file = $data_dir . '/schedule-' . $event_post_id . '.json';

// Section files.
$global_sections_file       = $data_dir . '/custom-sections.json';
$event_sections_file        = $data_dir . '/custom-sections-' . $event_post_id . '.json';
...
$site_settings_data = is_array( $site_settings_data ) ? $site_settings_data : array();
...
$global_sections      = is_array( $global_sections ) ? $global_sections : array();
$event_sections       = json_decode( file_get_contents( $event_sections_file ), true );
$event_sections       = is_array( $event_sections ) ? $event_sections : array();
...
// phpcs:enable WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents, WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
...
function get_video_embed_url( $url ) {
	...
	// Return the original URL if it is not a recognized format.
	return $url;
}
...
function render_custom_sections( $position, $all_sections ) {
	...
	foreach ( $sections_to_render as $section ) {
		...
		?>
			<section class="wp-block-group custom-section-added" id="custom-section-<?php echo esc_attr( $section_id ); ?>" style="padding: 48px 0;">
			<div class="container reveal" style="max-width: var(--container); margin: 0 auto; padding: 0 24px;">
				...
				<?php
				if ( 'media' === $section_type ) {
					// Render the media section.
					if ( 'photo' === $section['mediaType'] && ! empty( $section['photo_src'] ) ) {
						$image_src = $section['photo_src'];
						echo '<figure class="wp-block-image size-full"><img src="' . esc_url( $image_src ) . '" alt="' . esc_attr( $section['title'] ) . '" style="border-radius: 12px; width: 100%;"></figure>';
					} elseif ( 'video' === $section['mediaType'] && ! empty( $section['video_embed_src'] ) ) {
					...
					} elseif ( 'carousel' === $section['mediaType'] && ! empty( $section['carousel_images'] ) && is_array( $section['carousel_images'] ) ) {
						$carousel_id = 'carousel-' . $section['id'];
						$timer       = ! empty( $section['carousel_timer'] ) ? absint( $section['carousel_timer'] ) * 1000 : 5000;
						?>
							<div class="media-carousel" id="<?php echo esc_attr( $carousel_id ); ?>">
							<?php foreach ( $section['carousel_images'] as $index => $image_src ) : ?>
									<img src="<?php echo esc_url( $image_src ); ?>" class="carousel-slide" alt="Carousel image <?php echo esc_html( $index + 1 ); ?>" style="opacity: <?php echo esc_attr( 0 === $index ? '1' : '0' ); ?>;">
								<?php endforeach; ?>
							</div>
							<script>
								(function() {
								...
								setInterval(() => {
									...
									}, <?php echo esc_js( (string) $timer ); ?>);
								})();
							</script>
							<?php
					}
				} elseif ( 'full_width' === $layout ) {
					// Full-width layout.
					?>
					<div class="panel">
					...
					if ( 'photo' === $section['mediaType'] && ! empty( $section['photo_src'] ) ) {
						$image_src      = $section['photo_src'];
						$media_col_html = '<div class="wp-block-column" style="padding:0;"><figure class="wp-block-image size-full" style="margin:0;"><img src="' . esc_url( $image_src ) . '" alt="' . esc_attr( $section['title'] ) . ' media" style="border-radius: 12px; width: 100%; height: 100%; object-fit: cover;"></figure></div>';
					} elseif ( 'map' === $section['mediaType'] && ! empty( $section['map_embed_src'] ) ) {
...
// Dynamic event data for the hero.
