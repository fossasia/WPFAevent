...
class Wpfaevent_Eventyay_API_Client {
	...
	public function fetch_eventyay_event_speaker_program( $settings, $event_slug ) {
		...
		$program = $this->parser->normalize_eventyay_speakers_payload( $fetched['speakers'], $settings, $event_slug );
		...
		if ( ! is_wp_error( $featured_map ) && ! empty( $featured_map ) ) {
			$program['speakers'] = $this->parser->apply_eventyay_public_speaker_featured_map( $program['speakers'], $featured_map );
		}
